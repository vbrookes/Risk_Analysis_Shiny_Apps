################################################################################
###                 Expert opinion elicitation app: FREQUENCY              ###
################################################################################
...
ui <- fluidPage(
  ...
  actionButton("reset_button", "Reset Page"),
   ...
   # Application title
   titlePanel("Expert-opinion elicitation: Frequency"),

     sidebarPanel(
       textInput("tableName", label ="Data name", value="Default"),
       textInput("expert", label="expert", value="1"),
       numericInput("min", label="minimum", value="0", min = 0, max = 100000, step = 1),
       numericInput("ML", label="most likely", value="0", min = 0, max = 100000, step = 1),
       numericInput("max", label="maximum", value="0", min = 0, max = 100000, step = 1),
...
server <- function(input, output) {
  ...
  output$dotPlot <- renderPlot(
    if( input$addButton > 0) {
    dotplot.errors.expertRange(values$df, reference.line = NULL, main = input$tableName)}
    )
  ...
  output$densPlot <- renderPlot(
    ...
    ### Plot density plots
    plot(density(CombinedDistrib[[1]]), col="green3", xlim=range(Data[,2:4])+c(-.04,.04)*diff(range(Data[,2:4])), main = "", xlab = "", lwd = 2)
